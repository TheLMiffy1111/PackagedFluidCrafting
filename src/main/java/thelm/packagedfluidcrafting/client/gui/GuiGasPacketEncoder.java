...
import thelm.packagedfluidcrafting.client.GasRenderer;
...
public class GuiGasPacketEncoder extends GuiContainerTileBase<ContainerGasPacketEncoder> {

	public static final ResourceLocation BACKGROUND = new ResourceLocation("packagedfluidcrafting:textures/gui/gas_packet_encoder.png");
	public static final GasRenderer GAS_RENDERER = new GasRenderer(16, 52, 1);

	protected GuiTextField amountField;

	public GuiGasPacketEncoder(ContainerGasPacketEncoder container) {
		super(container);
	}
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		if(container.tile.currentGas != null) {
			GasStack stack = container.tile.currentGas.copy();
			stack.amount = container.tile.amount;
			GAS_RENDERER.render(guiLeft+80, guiTop+17, stack, container.tile.requiredAmount);
		}
	}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if(container.tile.isWorking && container.tile.currentGas != null && mouseX-guiLeft >= 80 && mouseY-guiTop >= 17 && mouseX-guiLeft <= 95 && mouseY-guiTop <= 68) {
			drawHoveringText(container.tile.currentGas.getGas().getLocalizedName()+" "+container.tile.amount+" / "+container.tile.requiredAmount+" mB", mouseX-guiLeft, mouseY-guiTop);
		}
	}
